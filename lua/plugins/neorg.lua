return {
	{
		...
		config = function()
			...
			vim.api.nvim_create_autocmd("Filetype", {
				...
				callback = function()
					...
					vim.keymap.set(
						"n",
						"<leader>td",
						"<cmd>Neorg keybind all core.qol.todo_items.todo.task_done<CR>",
						opts
					)
					vim.keymap.set(
						"n",
						"<leader>tu",
						"<cmd>Neorg keybind all core.qol.todo_items.todo.task_undone<CR>",
						opts
					)
					vim.keymap.set(
						"n",
						"<leader>tp",
						"<cmd>Neorg keybind all core.qol.todo_items.todo.task_pending<CR>",
						opts
					)
					vim.keymap.set(
						"n",
						"<leader>th",
						"<cmd>Neorg keybind all core.qol.todo_items.todo.task_on_hold<CR>",
						opts
					)
					vim.keymap.set(
						"n",
						"<leader>tc",
						"<cmd>Neorg keybind all core.qol.todo_items.todo.task_cancelled<CR>",
						opts
					)
					vim.keymap.set(
						"n",
						"<leader>tr",
						"<cmd>Neorg keybind all core.qol.todo_items.todo.task_recurring<CR>",
						opts
					)
					vim.keymap.set(
						"n",
						"<leader>ti",
						"<cmd>Neorg keybind all core.qol.todo_items.todo.task_important<CR>",
						opts
					)

					-- Navigation
					vim.keymap.set("n", "<leader>nl",
						"<cmd>Neorg keybind all core.dirman.new.location<CR>", opts)
					vim.keymap.set("n", "<leader>nf",
						"<cmd>Neorg keybind all core.dirman.new.file<CR>", opts)
					
					-- Create new note in notes folder
					vim.keymap.set("n", "<leader>nn", function()
						local workspace = require("neorg").modules.get_module("core.dirman").get_current_workspace()
						if workspace then
							local notes_path = workspace[2] .. "/notes"
							-- Create notes directory if it doesn't exist
							vim.fn.mkdir(notes_path, "p")
							-- Prompt for filename
							local filename = vim.fn.input("New note name: ")
							if filename ~= "" then
								if not filename:match("%.norg$") then
									filename = filename .. ".norg"
								end
					...
					vim.keymap.set("n", "<leader>nw", "<cmd>Telescope neorg switch_workspace<CR>",
						opts)
					vim.keymap.set("n", "<leader>nff", "<cmd>Telescope neorg find_norg_files<CR>",
						opts)
					vim.keymap.set("n", "<leader>nfl", "<cmd>Telescope neorg find_linkable<CR>", opts)
					vim.keymap.set("n", "<leader>nfh", "<cmd>Telescope neorg search_headings<CR>",
						opts)
					vim.keymap.set("n", "<leader>nfi", "<cmd>Telescope neorg insert_link<CR>", opts)
					vim.keymap.set("n", "<leader>nft", "<cmd>Telescope neorg find_project_tasks<CR>",
						opts)
					vim.keymap.set("n", "<leader>nfa",
						"<cmd>Telescope neorg find_aof_project_tasks<CR>", opts)
					vim.keymap.set("n", "<leader>nfc", "<cmd>Telescope neorg find_context_tasks<CR>",
						opts)
					vim.keymap.set("n", "<leader>nfr", "<cmd>Telescope neorg find_aof_tasks<CR>",
						opts)
				end,
			})
			...
			vim.keymap.set("n", "<leader>no", "<cmd>Neorg presenter start<CR>",
